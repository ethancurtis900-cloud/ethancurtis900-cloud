...
export function Services() {
  return (
    <section id="services" className="py-12 sm:py-16 md:py-20 bg-metro-950 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-96 h-96 bg-accent-600 rounded-full blur-3xl opacity-5"></div>

      <div className="relative max-w-7xl mx-auto px-6 sm:px-6 lg:px-8">
        <div className="text-center mb-10 sm:mb-10 md:mb-12">
          <span className="inline-block px-4 py-1.5 sm:px-3 sm:py-1.5 bg-accent-600/20 text-accent-300 rounded-full text-xs font-space-mono font-bold tracking-widest uppercase mb-3 sm:mb-2">
            Services
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-4xl lg:text-5xl font-900 text-white mb-4 sm:mb-3 md:mb-4 font-inter">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-4">
          {services.map((service, index) => (
            <div
              key={index}
              className="group relative bg-gradient-to-br from-metro-800 to-metro-900 rounded-lg p-6 sm:p-6 border border-metro-700 hover:border-accent-600 transition-all duration-300 hover:shadow-xl hover:shadow-accent-600/20 hover:-translate-y-1"
            >
              <div className="w-12 h-12 sm:w-12 sm:h-12 bg-gradient-to-br from-accent-600 to-accent-700 rounded-lg flex items-center justify-center mb-5 sm:mb-5 group-hover:scale-110 transition-transform">
                <service.icon className="w-6 h-6 sm:w-6 sm:h-6 text-white" />
              </div>

              <h3 className="text-xl sm:text-xl font-bold text-white mb-3 sm:mb-3 font-inter">
                {service.title}
              </h3>

              <p className="text-slate-300 leading-relaxed mb-5 sm:mb-5 text-base sm:text-base">
                {service.description}
              </p>

              <div className="space-y-2.5 sm:space-y-2">
                {service.features.map((feature, idx) => (
                  <div key={idx} className="flex items-center gap-2 text-sm sm:text-sm text-slate-300">
